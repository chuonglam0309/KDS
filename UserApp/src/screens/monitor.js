...
function Monitor(props) {
  ...
  const handleValue = async (ID) => {
    try {
      ...
      setDeviceType(rep[0].deviceType)
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const animatedStyle = {
    transform: [
      { translateX: interpolate(positionX.value, [-25, 25], [-50, 50]) },
      { translateY: interpolate(positionY.value, [-25, 25], [-50, 50]) },
    ],
  };
                  ...
                  <View>
                    {
                      (weight) == 0 ?
                        (
                          <Text style={[styles.text,]}>No gas leak detected</Text>
                        )
                        :
                        (
